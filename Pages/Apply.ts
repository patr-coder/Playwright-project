...
export default class ApplyPage {
  ...
  async typeFirstName(name: string) {
    await this.page.getByPlaceholder("First Name").type(name);
  }
  ...
  async typePhone(Phone: string) {
    await this.page.getByPlaceholder("Telephone").type(Phone);
  }
  async typePassword(Password: string) {
    await this.page.locator("input[name='password']").type(Password);
  }
  async typeRenterPassword(Password: string) {
    await this.page.getByPlaceholder("Password Confirm").type(Password);
  }
  async NotSubscribed() {
    return await this.page
      .locator("label[for='input-newsletter-no']")
      .isChecked();
  }
  async AgreeCheck() {
